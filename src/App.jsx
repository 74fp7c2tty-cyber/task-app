...
const App = () => {
    ...
    const checkNotifications = () => {
      ...
      if (timeStr === notifConfig.dailySummaryTime) {
        ...
        if (todayTasksCount > 0) {
          new Notification("TaskMaster: 今日の予定", { body: `今日は ${todayTasksCount}件 の作業スロットがあります。`, tag: 'daily-summary' });
        }
      }
      ...
      tasks.forEach(task => {
        ...
        if (diffHours > 0 && diffHours <= notifConfig.deadlineLeadHours && diffHours > (notifConfig.deadlineLeadHours - 0.02)) {
          new Notification("締切リマインド", { body: `「${task.title}」の締切まであと ${notifConfig.deadlineLeadHours} 時間です！`, tag: `deadline-${task.id}` });
        }
      });
  ...
  const sendTestNotification = () => {
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification("通知テスト", { body: "TaskMaster Pro からの通知は正常に機能しています！" });
    } else {
      alert("通知が許可されていないか、お使いのブラウザが対応していません。");
    }
  };
  ...
  const requestPermission = async () => {
    ...
    try {
      ...
      if (permission === 'granted') {
        setIsNotifSettingOpen(true);
      } else if (permission === 'denied') {
        alert("通知がブロックされています。ブラウザの設定から解除してください。");
      }
    } catch (e) {
      console.error("Permission request failed", e);
      alert("通知の許可リクエストに失敗しました。プレビュー環境では動作しない場合があります。");
    }
  };
  ...
  const recordWork = async (schId, taskId, actualH, progressDelta) => {
    ...
    if (!isComp && wasSafe && newSlots < (newRem - 0.1)) { setRevisionAlert({ taskId, message: `効率が落ちたため「${task.title}」の計画が不足しました。あと ${Math.ceil(newRem - newSlots)}枠 追加してください。` }); }
  };
  ...
  const deleteAllCompleted = async () => {
    ...
    for (const task of completedTasks) { await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', task.id)); }
  };
  ...
  const HomeView = () => (
    <div className="p-4 space-y-6 max-w-2xl mx-auto animate-in fade-in duration-500">
      <section className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-5 rounded-[24px] border border-gray-100 shadow-sm">
          <p className="text-[10px] text-gray-400 font-black uppercase mb-1">現在の平均効率</p>
      ...
      <section className="space-y-3">
        ...
        {groupedTodayTodo.length === 0 ? (
          <button onClick={() => setActiveTab('planner')} className="w-full bg-white border-2 border-dashed border-gray-100 rounded-[32px] p-12 text-center text-xs text-gray-400 hover:border-indigo-200">
            予定なし。計画を立てましょう
          </button>
        ) : (
  ...
  if (!user || user.isAnonymous) return (
    <div className="min-h-screen bg-indigo-800 flex flex-col items-center justify-center p-6 text-white text-center">
      ...
      {authErrorMessage && (
        <div className="mb-6 bg-red-500/20 border border-red-500/50 p-4 rounded-2xl text-xs text-red-100 flex flex-col items-center gap-2 max-w-sm">
    ...
    <div className="min-h-screen bg-gray-50 text-gray-950 font-sans selection:bg-indigo-100 overflow-x-hidden">
      <header className="bg-indigo-700 text-white p-5 sticky top-0 z-10 shadow-lg flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => notificationPermission === 'granted' ? setIsNotifSettingOpen(true) : requestPermission()} 
            className={`${notificationPermission === 'granted' ? 'bg-green-500/20 text-green-300' : 'bg-white/10 text-white'} p-2 rounded-full transition-all active:scale-90`}
          >
        ...
        {activeTab === 'completed' && (
          <div className="p-4 space-y-4 max-w-2xl mx-auto">
            <div className="flex justify-between items-center px-2">
              ...
              {tasks.filter(t => t.completed).length > 0 && <button onClick={deleteAllCompleted} className="bg-red-50 text-red-500 text-[10px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1 active:scale-95 transition-all shadow-sm"><Trash2 size={12} /> 一括削除</button>}
            </div>
        ...
        {[
          ...
          <button 
            ...
            className={`relative flex flex-col items-center transition-all duration-300 px-3 py-1 ${
              tab.special 
                ? 'bg-indigo-600 text-white w-20 h-20 -mt-10 flex items-center justify-center rounded-full shadow-indigo-300 shadow-2xl active:scale-90 ring-8 ring-white hover:bg-indigo-700' 
      ...
      {isNotifSettingOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-8 space-y-6 animate-in zoom-in duration-300 shadow-2xl">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-black flex items-center gap-2"><SettingsIcon size={20}/>通知設定</h2>
              <button onClick={() => setIsNotifSettingOpen(false)} className="p-2 bg-gray-50 rounded-full"><X size={18}/></button>
            </div>
            
            <div className="space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">期限前リマインド</label>
                <div className="flex items-center gap-3 bg-gray-50 p-4 rounded-2xl">
                  <Clock size={18} className="text-indigo-500" />
                  <select 
                    className="flex-1 bg-transparent font-bold outline-none" 
                    value={notifConfig.deadlineLeadHours}
                    onChange={(e) => saveNotifSettings({...notifConfig, deadlineLeadHours: parseInt(e.target.value)})}
                  >
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">今日のまとめ通知</label>
                <div className="flex items-center gap-3 bg-gray-50 p-4 rounded-2xl">
                  <Calendar size={18} className="text-indigo-500" />
                  <input 
                    type="time" 
                    className="flex-1 bg-transparent font-bold outline-none"
                    value={notifConfig.dailySummaryTime}
                    onChange={(e) => saveNotifSettings({...notifConfig, dailySummaryTime: e.target.value})}
                  />
                </div>
              </div>
              ...
              <button onClick={sendTestNotification} className="w-full flex items-center justify-center gap-2 py-3 border-2 border-indigo-100 rounded-2xl text-indigo-600 font-bold text-sm hover:bg-indigo-50 transition-colors">
                <SendHorizontal size={18} /> テスト通知を送る
              </button>
            </div>
            
            <div className="space-y-3">
              <button onClick={() => setIsNotifSettingOpen(false)} className="w-full bg-indigo-600 text-white p-4 rounded-3xl font-black shadow-lg">閉じる</button>
              {saveFeedback && <p className="text-center text-[10px] text-green-500 font-bold animate-pulse">設定を同期しました ✓</p>}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full max-w-md rounded-t-[40px] sm:rounded-[32px] p-8 space-y-6 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-gray-400 ml-1">作業時間 (h)</label>
